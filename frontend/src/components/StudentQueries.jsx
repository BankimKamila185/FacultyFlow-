    ...
    return (
        <div className="sq-container">
            <style>{`
                ...
                .btn-vibrant-sm:hover { transform: scale(1.02); box-shadow: 0 6px 16px var(--primary-glow); }
            `}</style>

            <div className="sq-header">
                <h2>Direct Student Pulse</h2>
                <p>Categorized queries and requests requiring immediate faculty attention.</p>
            </div>
            
            <div className="queries-grid">
                {loading ? (
                    <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--text-dim)', gridColumn: '1/-1' }}>
                        <p style={{ marginTop: '1rem', fontWeight: 600 }}>Analyzing student queries...</p>
                    </div>
                ) : queries.length === 0 ? (
                    <div className="placeholder-section" style={{ gridColumn: '1/-1' }}>
                        <div className="placeholder-icon">📫</div>
                        <h3>All Clear</h3>
                        <p>No pending student queries at the moment.</p>
                    </div>
                ) : queries.map(q => (
                    <div className="q-card" key={q.id}>
                        <div className="q-top">
                            ...
                            <span className={`q-status ${q.isRead ? 'status-read' : 'status-new'}`}>
                                {q.isRead ? 'Resolved' : 'Priority'}
                            </span>
                        </div>

                        <div className="q-content">
                            <div className="q-subject">{q.subject}</div>
                            <p className="q-summary">{q.aiSummary || q.bodySnippet || 'Processing query content...'}</p>
                        </div>
